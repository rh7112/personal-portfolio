        ...
        <div className="rounded-3xl border border-stone-900/10 bg-white/70 p-8 dark:border-white/10 dark:bg-stone-900/60 lg:p-10">
          ...
          <div className="mt-8 grid gap-6 lg:grid-cols-3">
            {featuredProjects.map((project) => {
              const companyColor = resolveCompanyColor(project.companySlug, employers);
              return (
                <article
                  key={project.title}
                  className={`overflow-hidden rounded-3xl border ${getCompanyCardClass(companyColor)}`}
                >
            ...
            {projects.map((project) => {
              ...
              return (
                <span
                  ...
                  className={`rounded-full border px-3 py-2 text-sm text-stone-700 dark:text-stone-200 ${getCompanyCardClass(companyColor)}`}
                >
                  {project.title}
                </span>
              );
